function ScgFromScImpl(_sandbox, _editor, aMapping) {
  ...
  function randomContourPolygon() {
    let centerPos = randomPos();
    return [
      new SCg.Vector3(centerPos.x - 100, centerPos.y - 100, centerPos.z),
      new SCg.Vector3(centerPos.x - 100, centerPos.y + 100, centerPos.z),
      new SCg.Vector3(centerPos.x + 100, centerPos.y + 100, centerPos.z),
      new SCg.Vector3(centerPos.x + 100, centerPos.y - 100, centerPos.z),
    ];
  ...
  function _addNodeToScene(type, addr) {
  ...
  function _addEdgeToScene(bObj, eObj, type, addr) {
  ...
  function _addContourToScene(type, addr) {
    let modelContour = SCg.Creator.createCounter(randomContourPolygon());
    editor.scene.appendObject(modelContour);
    editor.scene.objects[addr] = modelContour;
    modelContour.sc_type = type;
    modelContour.sc_addr = addr;
    resolveIdtf(addr, modelContour);
  }
